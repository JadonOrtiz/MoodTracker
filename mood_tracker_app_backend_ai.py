"""
Mood Tracker App with backend-powered AI suggestions.
...
- No API key or model fields in the user interface
- The desktop app talks only to your local/backend endpoint

How AI works:
1. Turn on "Use AI for content" in Preferences.
2. Start the backend service.
3. The app sends the current mood snapshot to the backend.
4. The backend calls the AI provider and returns clean JSON.

The desktop app never stores or asks the user for an AI API key.
"""
...
class PreferencesScreen(BaseScreen):
    def __init__(self, parent: ttk.Frame, app: "MoodTrackerApp") -> None:
        ...
        ttk.Checkbutton(ai_form, text="Use AI for content", variable=self.use_ai_var).grid(row=0, column=0, sticky="w", pady=6)
        ttk.Label(
            ai_form,
            text="Backend endpoint: http://127.0.0.1:8000/generate\nAI keys stay in the backend, not in this app.",
            style="Muted.TLabel",
            justify="left",
